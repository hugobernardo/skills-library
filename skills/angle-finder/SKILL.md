...
Content Angle Finder is a **standalone, self-contained skill**. It includes its own research engine and bundled library — no dependencies on other skills.

Use this skill when the output is a single long-form piece (LinkedIn longform, Substack essay, X article, newsletter).
...
Do NOT use when:
- User wants multiple independent content pieces from one topic — this skill produces ingredients for ONE unified piece
...
This skill is self-contained — everything it needs lives inside `~/.claude/skills/marketing-angle-finder/`. To unlock the full research engine, the user needs three API keys.

### 1. Install the skill

Drop the `marketing-angle-finder/` folder into `~/.claude/skills/`. That's it — Claude Code picks it up automatically.

### 2. Configure API keys

Create `~/.config/marketing-angle-finder/.env` with the following:
...
```bash
python3 ~/.claude/skills/marketing-angle-finder/scripts/singleangle-research.py --check
```
...
- **No audience profile is installed with the skill.** You can optionally provide audience context each time you invoke this skill — see Inputs below.
...
- A seed tweet/article URL — the skill will treat it as the trigger source
...
```bash
python3 ~/.claude/skills/marketing-angle-finder/scripts/singleangle-research.py "<topic>" --emit=compact [--deep] [--days=N]
```
...
- Save structured research to `~/.local/share/marketing-angle-finder/out/research.md`
...
Save to `~/.local/share/marketing-angle-finder/out/<topic-slug>.md` with the section structure above.
